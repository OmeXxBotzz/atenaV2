...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    ...
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let str = `
Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Kyoto', 'Tokyo', 'Akihabara', 'Hokkaido', 'Osaka', 'Kawaguchi', 'Gunung Kembar', 'Alam Neraka', 'Hutan Kematian', 'Bulan', 'Padang Gurun', 'Savana', 'Isekai', '...'])} dan mendapatkan
*exp:* ${exp} 
*uang:* ${uang}
...
`.trim()
            ...
            conn.sendButton(m.chat, str, watermark, 'HEAL', '.heal', m)
            if (mythic > 0) {
