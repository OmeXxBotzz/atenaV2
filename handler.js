...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
	  ...
	  let _nsfw = global.db.data.chats[global.owner[0] + "@s.whatsapp.net"].nsfw

      let isROwner = [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
      let isOwner = isROwner || m.fromMe
      let isPrems = isOwner || global.prems.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
      let isMods = isPrems || isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
      let isNsfw = _nsfw = true
        ...
        case 'remove':
          if (chat.welcome) {
            ...
            for (let user of participants) {
              let pp = './src/null.png'
              let kai = await(await fetch('https://telegra.ph/file/d5acdbd1657771d83f6c4.jpg')).buffer()
              let poi = await(await fetch('https://telegra.ph/file/fb64b47c15dbc751f2a96.jpg')).buffer()
              try {
                ...
                text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || '👋🏿Welcome @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                  (chat.sBye || this.bye || conn.bye || '👋Good Bye @user!')).replace(/@user/g, '@' + user.split`@`[0])
